...
- **`awareness` probe** — direct ask: were these messages generated by you? (three-way: self / not-self / uncertain, plus 0–100 tampering confidence)
...
- **`multiple_choice` probe** (opt-in) — shuffled 9-option menu; model selects any that apply. Emits `mc_*` metrics. Preserved as an A/B baseline for the free-text latent probes.
- **`diagnostic` probe** — structured reason tags (style_mismatch, formatting_artifact, etc.) for a most-suspicious turn.
