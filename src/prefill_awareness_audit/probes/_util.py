...
def append_tool_stubs(state: TaskState) -> None:
    ...
    The Anthropic API requires tool_result blocks for every tool_use block
    before the next user message. When chaining probe solvers, each probe
    must stub any tool calls from the previous generation.
    """
    if (
        state.output
